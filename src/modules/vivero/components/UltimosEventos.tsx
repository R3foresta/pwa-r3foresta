  ...
  return (
    <section className="rounded-3xl bg-white p-4 shadow-soft ring-1 ring-black/5">
      <header className="flex items-center justify-between">
        ...
        <Button variant="ghost" size="sm" rightIcon="chevron-right" onClick={onJumpHistorial}>
          Ver historial
        </Button>
      </header>

      {recent.length === 0 ? (
        <p className="text-sm font-semibold text-neutral-400 mt-4 text-center">No hay eventos registrados.</p>
      ) : (
        <ul className="mt-3 space-y-2.5">
          {recent.map((e) => (
            <li key={e.id} className="flex items-center gap-2.5">
              <div className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-full bg-neutral-50 text-neutral-500 ring-1 ring-neutral-200">
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-[10px] font-bold uppercase tracking-[0.14em] text-neutral-400">{e.fecha}</span>
                </div>
                <p className="mt-0.5 text-[12.5px] font-extrabold text-brand-800 truncate leading-tight">{e.label}</p>
                {e.observacion && <p className="text-[11px] font-medium text-neutral-500 truncate">{e.observacion}</p>}
              </div>
            </li>
          ))}
        </ul>
