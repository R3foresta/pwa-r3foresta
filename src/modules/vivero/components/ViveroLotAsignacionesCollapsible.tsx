  ...
  return (
    <div className={className}>
      <button
        ...
        className="flex w-full items-center justify-between text-xs font-bold text-neutral-500 hover:text-neutral-700"
      >
        <div className="flex items-center gap-1.5">
          <Icon name="package" className="h-4 w-4 text-neutral-400" />
          <span className="text-sm font-extrabold text-brand-800">Asignaciones activas ({cantidadAsignacionesActivas})</span>
        </div>
        <Icon
          name="chevron-down"
          className={`h-4 w-4 text-neutral-400 transition-transform duration-200 ${showAsignaciones ? 'rotate-180' : ''}`}
        />
      </button>
      ...
      {showAsignaciones && (
        <div className="mt-3 space-y-2">
          {cantidadAsignacionesActivas === 0 ? (
            <p className="text-[11px] font-semibold text-neutral-400 text-center py-2">No hay asignaciones activas para este lote.</p>
          ) : loadingAsig ? (
            ...
            <ul className="space-y-2">
              {asignaciones.map((asig) => (
                <li key={asig.id} className="flex items-center gap-2.5 rounded-2xl bg-neutral-50/50 p-2.5 ring-1 ring-black/5">
                  <div className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full bg-neutral-100 text-neutral-500 ring-1 ring-neutral-200">
