    ...
    <section className="rounded-3xl bg-white p-5 shadow-sm ring-1 ring-neutral-100">
      <header className="flex items-start justify-between gap-3">
        <div>
          ...
          <h3 className="mt-0.5 text-base font-extrabold text-brand-950 leading-tight">Supervivencia y<br/>composición actual</h3>
        </div>
        <div className="text-right shrink-0">
          <p className="text-[9px] font-black uppercase tracking-widest text-neutral-500">En vivero</p>
          <p className="text-sm font-black text-brand-950 leading-none mt-0.5">
            {detail.diasDesdeInicio ?? 0} <span className="font-bold text-neutral-500">días</span>
          </p>
        </div>
      </header>
      ...
      <div className="mt-5 flex items-end justify-between gap-2">
        <div>
          <p className="text-[9px] font-black uppercase tracking-widest text-brand-950">Supervivencia</p>
          <p className="text-5xl font-black text-brand-950 leading-none tracking-tighter mt-1">{supervivencia}%</p>
        </div>
        <div className="text-right pb-1">
          <p className="text-[9px] font-black uppercase tracking-widest text-neutral-500">Vivas hoy</p>
          <p className="text-2xl font-black text-brand-950 leading-none mt-1">
            {vivasHoy.toLocaleString('es-BO')} <span className="text-sm font-bold text-neutral-400">/ {plantasIniciales.toLocaleString('es-BO')}</span>
          </p>
        </div>
      </div>
