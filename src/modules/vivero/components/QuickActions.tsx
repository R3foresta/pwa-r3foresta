...
export default function QuickActions({ detail, onJumpEvidencia }: QuickActionsProps) {
  ...
  if (closed) {
    return (
      <div className="rounded-2xl bg-neutral-100 p-4 shadow-soft ring-1 ring-black/5 flex items-center gap-3">
        <Icon name={"shield" as IconName} className="h-5 w-5 text-neutral-500 shrink-0" />
        <div>
          <p className="text-xs font-extrabold text-neutral-700 uppercase tracking-wide">Lote finalizado</p>
          <p className="text-[11px] font-medium text-neutral-500 leading-snug">No admite nuevos eventos operativos.</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <section>
      ...
      <div className="grid grid-cols-4 gap-2">
        {/* Tiles de acción rápida (ícono + label, layout vertical): control a medida,
            no calza en <Button>; disabled por dominio preservado. (gotcha §6.5) */}
        {actions.map((a) => (
          <button
            key={a.key}
            type="button"
            ...
            className={`flex flex-col items-center justify-center gap-1.5 rounded-2xl bg-white px-2 py-3 shadow-soft ring-1 ring-black/5 transition-all active:scale-[0.97] ${
              a.disabled 
                ? 'opacity-40 cursor-not-allowed bg-neutral-50 ring-transparent' 
                : 'hover:ring-brand-300'
            }`}
          >
            <div className={`flex h-9 w-9 items-center justify-center rounded-2xl ${a.disabled ? 'bg-neutral-200 text-neutral-400' : 'bg-brand-50 text-brand-600'}`}>
