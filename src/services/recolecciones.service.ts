...
export class RecoleccionesService {
  ...
  private static normalizeListResponse(payload: unknown): ListRecoleccionesResult {
    if (Array.isArray(payload)) {
      return {
        success: true,
        data: payload as Recoleccion[],
        pagination: this.buildPagination(undefined, payload.length),
      }
    }

    const envelope = payload as ApiEnvelope<Recoleccion[] | { items: Recoleccion[] }>
  ...
  static async list(filters?: RecoleccionFilters): Promise<ListRecoleccionesResult> {
  ...
  static async getById(id: number): Promise<{ success: boolean; data: Recoleccion }> {
    ...
    const payload = await this.parseJsonResponse<ApiEnvelope<Recoleccion> | Recoleccion>(response)

    if ('data' in (payload as ApiEnvelope<Recoleccion>) && (payload as ApiEnvelope<Recoleccion>).data) {
      return {
        success: Boolean((payload as ApiEnvelope<Recoleccion>).success ?? true),
        data: (payload as ApiEnvelope<Recoleccion>).data as Recoleccion,
      }
    }
  ...
  static async submit(id: number): Promise<{ success: boolean; data: Recoleccion }> {
  ...
  static async approve(id: number): Promise<{ success: boolean; data: Recoleccion }> {
  ...
  private static validateCreatePayload(data: CreateRecoleccionDto) {
